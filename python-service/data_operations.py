...
def suggest_initial_charts(
    data_or_summary: list[dict[str, Any]] | dict[str, Any]
) -> list[dict[str, Any]]:
    """
...
def aggregate_data(
    ...
    any(word in user_intent_lower for word in ['p90', 'p95', 'p99', 'percentile', 'top 10%', 'top 5%', 'top 1%'])
    ...
    any(word in user_intent_lower for word in ['variability', 'variance', 'std', 'standard deviation', 'stability', 'spread'])
    ...
    if index_column not in result_df.columns and columns_to_preserve:
        ...
        for _idx, row in result_df.iterrows():
            ...
            if max_status is not None:
                ...
                for _orig_idx, orig_row in df.iterrows():
                    match = True
    ...
    for col in columns_to_analyze:
        ...
        if method == "iqr":
            ...
            try:
                ...
                for idx, (value, pred) in enumerate(zip(df[col], predictions, strict=False)):
                    if pd.isna(value):
                        continue
                    if pred == -1:  # Outlier
                        column_outliers.append({
                ...
                raise ValueError("scikit-learn is required for isolation_forest method") from None
            ...
            try:
                ...
                raise ValueError("scikit-learn is required for local_outlier_factor method") from None
