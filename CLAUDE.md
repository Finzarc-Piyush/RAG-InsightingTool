...
9. **Per-session mutex `withSessionWriteLock`** ([server/lib/sessionWriteLock.ts](server/lib/sessionWriteLock.ts), Wave A2) is THE unified write mutex for all RMW on a `ChatDocument`. Single-instance correctness only.
