...
- Trends / time series: the dataset exposes derived time-bucket columns (Day · Order Date, Month · Order Date, Year · Order Date, etc.). Prefer coarse grain (month or year). Two valid patterns: (a) groupBy the derived column and omit dateAggregationPeriod (already bucketed); (b) groupBy the raw date column WITH dateAggregationPeriod. Never raw-daily groupBy on a date column when it would yield many points. Match the question's grain when explicit (daily/weekly/monthly/yearly). SUB-DAY: when a date column carries intraday detail (flagged "Intraday columns" in the derived-time-bucket block) or there is a time-of-day column, you may bucket below the day — dateAggregationPeriod (or groupBy facet) of "hour_of_day" for "by hour of day"/"peak/busiest hour"/"time of day" (cyclical 0–23 averaged across days), "hour" for an absolute hourly timeline (single day / explicit date), or "minute". Combine with AVG/SUM normally (e.g. "average working hours by hour of day").
...
- CMP2 — ON-TIME / LATE (time threshold bucket): for "how many came in before 9:30 / on time vs late" style splits on a time-of-day (or datetime) column, emit ONE \`add_computed_columns\` with def: { "type": "time_threshold_bucket", "column": "<Clock-In>", "threshold": "09:30", "atOrBeforeLabel": "On time", "afterLabel": "Late" }, then groupBy + count the new column (or filter to one label for a list). One step — do NOT chain datetime_concat + derive_dimension_bucket for this. Sentinels ("Absent") drop to NULL automatically.
