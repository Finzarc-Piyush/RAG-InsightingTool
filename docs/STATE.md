...
- **Incremental data refresh ("Update data"):** v1 SHIPPED behind `INCREMENTAL_REFRESH_ENABLED` (server) + `VITE_INCREMENTAL_REFRESH_ENABLED` (client), both default OFF. Attach an updated file (or "Fetch latest" from Snowflake) to an existing chat/dashboard → the whole analysis regenerates on the new data. **Replace** (new supersedes) or **Append** (old + new → full combined). Built as Waves WR0–WR8 by re-targeting the automation capture→replay engine (`replayRecipe` extracted from `replayAutomation`); dashboard updates in place. Plan `/Users/tida/.claude/plans/an-extremely-new-functionality-pure-graham.md`. Architecture: [`docs/architecture/incremental-refresh.md`](architecture/incremental-refresh.md). **v1.1 backlog:** April-vs-May compare view; fresh-planner re-analysis; user-initiated rollback UI (data versions retained — needs a rollback endpoint); scheduled Snowflake refresh; flagged Parquet sibling write on refresh.
