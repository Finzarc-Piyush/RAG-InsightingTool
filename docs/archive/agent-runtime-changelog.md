# agent-runtime — archived subsystem changelog

> Relocated out of `docs/architecture/agent-runtime.md` (2026-06-14) to keep that routing
> doc cheap to /load. Historical detail only — the authoritative wave log is
> `docs/WAVES.md`. Not scanned by check-doc-refs (archive = historical).

---
## Recent changes

- **Domain-aware dashboards: valid universe + real pivots (2026-06-10)** — fixes all-zero pivots, structural-zero blindness, generic narratives. **Pivot (client):** `PivotAgg` gains identity `'first'` and `applyAgg` resolves fields case/separator-tolerantly ([buildPivotModel.ts](../../client/src/lib/pivot/buildPivotModel.ts)); [chartSpecToPivotConfig.ts](../../client/src/components/charts/chartSpecToPivotConfig.ts) maps derived rate/ratio columns (`/_rate\b|_ratio\b|…/`) to `'first'` so a per-group rate is displayed, not re-summed to 0. **Axis:** [axisScaling.ts](../../server/lib/axisScaling.ts) returns `[0,1]` for an all-zero series (no negative percent ticks). **Valid universe (keystone):** new [inferMetricApplicability.ts](../../server/lib/inferMetricApplicability.ts) cross-tabs a boolean indicator's positives against categoricals and (via name-affinity + plan/type hint) picks the gate column + in-scope values (PJP Adherence → `PJP Planned Type ∈ [Market Working]`), stamped as `indicator.applicabilityScope` ([schema.ts](../../server/shared/schema.ts)) at upload ([uploadQueue.ts](../../server/utils/uploadQueue.ts)) with a guarded lazy build-time fallback in [agentLoop.service.ts](../../server/lib/agents/runtime/agentLoop.service.ts). Rate steps AND the scope into numerator+denominator (`scopePredicateCells`, [booleanIndicatorRateRepair.ts](../../server/lib/agents/runtime/booleanIndicatorRateRepair.ts) + `makeCoverageRateStep`) → adherence = 33.5% on Market-Working days, not 21% diluted; degenerate breakdowns by the gate column are skipped ([dashboardCoverageGate.ts](../../server/lib/agents/runtime/dashboardCoverageGate.ts)). **Narrative:** anti-meta-hedge + structural-zero rules in [insightGenerator.ts](../../server/lib/insightGenerator.ts) + [narratorAgent.ts](../../server/lib/agents/runtime/narratorAgent.ts). **Investigation:** `filterSpawnedQuestions` applied at the open-questions display surface ([buildInvestigationSummary.ts](../../server/lib/agents/runtime/buildInvestigationSummary.ts)). Tests: `inferMetricApplicability`, `axisScaling.multiSeries`, `dashboardCoverageGate` (W5/W6), client `chartToPivotIntegration.vitest`.
- **Manager-centric dashboard redesign (2026-06-10)** — reshapes dashboards around management-by-exception. **MW1** new [filterSpawnedQuestions.ts](../../server/lib/agents/runtime/filterSpawnedQuestions.ts) at the spawned-question chokepoint ([agentLoop.service.ts](../../server/lib/agents/runtime/agentLoop.service.ts) ~2990) hard-drops random-sample / duplicate / per-identifier-grouping chips (+ reflector W8 prompt forbids them). **MW2** rate-first framing: dashboard breakdowns use size-normalized `mean` not raw `sum` ([dashboardCoverageGate.ts](../../server/lib/agents/runtime/dashboardCoverageGate.ts) `makeCoverageBuildChartStep`, [dashboardFeatureSweep.ts](../../server/lib/agents/runtime/dashboardFeatureSweep.ts) `tryBuildChart`) + a planner DASHBOARD METRIC FRAMING rule preferring rates/ratios/averages. **MW3** `chartSpec.sortDirection`/`maxRows` honoured by `processChartData` ([chartGenerator.ts](../../server/lib/chartGenerator.ts)) → bottom-N "worst performers" + no silent truncation; client adds a "View all N records" CTA into the sortable table ([ChartTileBody.tsx](../../client/src/pages/Dashboard/Components/ChartTileBody.tsx)). **MW4** new [computeAttentionAreas.ts](../../server/lib/agents/runtime/computeAttentionAreas.ts) flags below-org-average units from the breakdown charts (>1 SD below = red, else amber); stamped onto `DashboardSpec.attentionAreas` ([buildDashboard.ts](../../server/lib/agents/runtime/buildDashboard.ts), persisted via [dashboard.model.ts](../../server/models/dashboard.model.ts)) and rendered as an "Attention areas" RAG callout in [DashboardSummaryBand.tsx](../../client/src/pages/Dashboard/Components/DashboardSummaryBand.tsx). **MW5** new [attachReferenceLines.ts](../../server/lib/agents/runtime/attachReferenceLines.ts) adds an "Org avg" `reference-line` `_autoLayer` (client resolves `value:"mean"`) to each breakdown. **MW6** clicking an attention area cross-filters the dashboard to that unit ([DashboardView.tsx](../../client/src/pages/Dashboard/Components/DashboardView.tsx)). Tests: `filterSpawnedQuestions`, `chartSortDirectionMaxRows.test.ts`, `computeAttentionAreas`, `attachReferenceLines` (+ extended coverage/sweep) + client `summaryBandData.vitest`.
- **Dashboard data-access + multi-KPI (2026-06-10)** — fixes "ASM only partially shown in the snippet" / "only one temporal bucket" caveats + makes "build a pjp dashboard" multi-KPI. **W1** [`buildSynthesisContext.ts`](../../server/lib/agents/runtime/buildSynthesisContext.ts) now surfaces the COMPLETE rows of small (≤50-row) aggregated steps to the narrator via the `result` on `structuredObservations` (rendered inside the DATA UNDERSTANDING block) — the narrator previously saw only row *counts* and hedged that a full ranking "cannot be stated". **W2** widened the binding caps for small aggregations: the blackboard finding `detail` cap (800→3000) in [`agentLoop.service.ts`](../../server/lib/agents/runtime/agentLoop.service.ts) and the `execute_query_plan` `OBSERVATION_TOP_K`/JSON cap (30→all-rows, 3500→8000 for ≤50-row results) in [`registerTools.ts`](../../server/lib/agents/runtime/tools/registerTools.ts). **W3** day-grain trend: [`dashboardFeatureSweep.ts`](../../server/lib/agents/runtime/dashboardFeatureSweep.ts) `pickStrongestDateColumn` validates a Month facet's bucket count via `distinctBucketsForGrain` and falls to the span-appropriate finer facet (Day on a single-month span); [`patchExecuteQueryPlanTrendGrain`](../../server/lib/queryPlanTemporalPatch.ts) gains an `isDashboard` opt so dashboard trend steps get grain refinement without "trend"/"over time" wording (wired in [`planner.ts`](../../server/lib/agents/runtime/planner.ts)). **W5** [`dashboardCoverageGate.ts`](../../server/lib/agents/runtime/dashboardCoverageGate.ts) emits a TOP-N (`limit 15`) rate leaderboard for high-cardinality entity dims (TSOE) on boolean outcomes — previously dropped. **W6** multi-KPI: `analysisBriefSchema.outlineMetrics?: string[]` ([`schema.ts`](../../server/shared/schema.ts)); `ensureDashboardOutlineMetrics` seeds it from the dataset's boolean-indicator cluster for indicator-centric dashboards (data-derived, no LLM); the coverage gate charts each secondary KPI by the key low-card dims (bounded). `ensureDashboardOutcomeMetric` is now also applied on the merged pre-planner lane ([`runHypothesisAndBrief.ts`](../../server/lib/agents/runtime/runHypothesisAndBrief.ts) — was a latent gap). Tests: `buildSynthesisContext.test.ts`, `lowCardObservationCap.test.ts`, `dashboardFeatureSweep.test.ts`, `dashboardCoverageGate.test.ts`, `multiKpiCoverage.test.ts`, `queryPlanTrendFacetGrainPatchT4.test.ts`.
- **Wave WS2-pre-classify-parallel · concurrent kickoff of the three pre-classify operations in `chatStream.service.ts:processStreamChat` that have no data dependency on earlier results (2026-05-21)** — closes the first-name on the post-W61 "Outside-W61 deferred candidates" punch list. Pre-wave the pre-classify block at lines 893-1170 of the 2,398-LOC host ran strictly sequentially: schemaBind → analyzeChatWithColumns (depends on schemaBind) → parseUserQuery (independent) → loadEnabledDomainContext (independent) → classifyMode (depends on domainContext). New pure helper [`kickOffPreClassifyWork<S, Q, D>`](../../server/services/chat/chatStreamPreClassifyKickoff.ts) (40 LOC) takes three thunk-shaped deps + returns `{ schemaBinding, parsedQuery, domainContext }` promises with `.catch(() => null)` on the two pre-wave-already-swallowed (parseUserQuery / loadEnabledDomainContext) and bare-throw passthrough on schemaBind. [`chatStream.service.ts`](../../server/services/chat/chatStream.service.ts) hoists a single `kickoff = kickOffPreClassifyWork({...})` const at line ~893 (above the first SSE thinking-step emit), then collapses three downstream consumption sites by replacing inline `await` calls with `await kickoff.X`. All SSE thinking-step emissions stay byte-identical at their original line positions — only `await` targets and surrounding try/catch shape change, so on-wire ordering is preserved verbatim and the W31 sse-contract pin in `tests/sseContract.test.ts` is not perturbed. Estimated latency floor savings ~300-500 ms on cold-cache LLM calls. 7 new behavioural tests in [`chatStreamPreClassifyKickoff.test.ts`](../../server/tests/chatStreamPreClassifyKickoff.test.ts) pin concurrent-kickoff timing + three permutations of error-policy + value pass-through + once-per-kickoff memoisation. See `docs/WAVES.md` for full entry.
- **Wave WV4-bucket · effectMagnitude on `run_correlation` via Cohen's |r| (2026-05-20)** — direct follow-on to WQ8. New pure helper [`bucketCorrelationR(r)`](../../server/lib/correlationMath.ts) in `correlationMath.ts` mirrors `bucketCohensD` / `bucketCramersV` from `significanceTests.ts`. Standard Cohen (1988) thresholds on |r|: `< 0.1` → `negligible` / `< 0.3` → `small` / `< 0.5` → `medium` / `≥ 0.5` → `large`. Returns `null` on non-finite / `|r| > 1`. The WV4 block in [`registerTools.ts`](../../server/lib/agents/runtime/tools/registerTools.ts) reads `strongest.correlation`, buckets it, and assigns the result to `evidence.effectMagnitude` under a truthy guard alongside the existing `n` + `rSquared` carriage. Closes the WQ8 dormancy of "real but trivial" correlation findings landing at MEDIUM instead of LOW. 16 new tests in [`runCorrelationEffectMagnitudeWV4Bucket.test.ts`](../../server/tests/runCorrelationEffectMagnitudeWV4Bucket.test.ts). See `docs/WAVES.md` for full entry.
- **Wave WQ8 · effect-size as fifth FindingEvidence field (2026-05-20)** — plumbs the WV5 significance-test tool's categorical `effect_magnitude` bucket through the WV/WW/WQ deterministic-floor trinity into the WQ1 classifier. [`scaleNarrativeByConfidence.ts`](../../server/lib/agents/runtime/scaleNarrativeByConfidence.ts) `FindingEvidence` gains `effectMagnitude?: "negligible" | "small" | "medium" | "large"` (the fifth channel after `n` / `pValue` / `rSquared` / `ciRelativeWidth`; the pre-existing `magnitude?: number` field stays untouched as the numeric-value slot). `assessConfidence` extended in four places: (i) `hasAnyEvidence` includes `effectMagnitude`; (ii) LOW hard-fail block adds `effectMagnitude === "negligible"` with reason `"negligible effect size"` — the load-bearing wedge that downgrades "significant but trivial" findings even with strong n / p / CI / R²; (iii) HIGH all-conditions gate requires `effectMagnitude` to be `undefined` / `"medium"` / `"large"`, with `"large effect size"` / `"medium effect size"` surfacing as reasons; (iv) MEDIUM catch-all surfaces `"small effect size"` / `"medium effect size"` reasons. [`formatFindingEvidence.ts`](../../server/lib/agents/runtime/formatFindingEvidence.ts) `formatEvidenceForFindingDetail` emits canonical `effect = <bucket>` token after the CI block. [`narratorHintsBlock.ts`](../../server/lib/agents/runtime/narratorHintsBlock.ts) `extractFindingEvidence` regex tolerates `effect = X` / `effect: X` / `effect-size: X` / `effect_magnitude: X`. Mental model: p-value answers "is it noise?"; effect-size answers "does it matter?" — both must clear the bar for HIGH. 22 new tests in [`effectMagnitudeFindingEvidenceWQ8.test.ts`](../../server/tests/effectMagnitudeFindingEvidenceWQ8.test.ts); WV5 test regexes relaxed to allow optional trailing `; effect = X`. See `docs/WAVES.md` for full entry.
- **Wave WI2-server · POST /api/insight/regen endpoint (2026-05-18)** — adds a new MINI-tier LLM-call lane through `callLlm`. New [`LLM_PURPOSE.INSIGHT_REGEN`](../../server/lib/agents/runtime/llmCallPurpose.ts) (`"insight_regen"`) declared with `[LLM_PURPOSE.INSIGHT_REGEN]: "MINI"` in `PURPOSE_TO_CATEGORY`. The dashboard tile insight-regeneration endpoint at [`server/routes/insightRegen.ts`](../../server/routes/insightRegen.ts) routes its single LLM call through `callLlm({ ..., max_tokens: 380 }, { purpose: LLM_PURPOSE.INSIGHT_REGEN })` — `resolveModelFor` then picks the model from `OPENAI_MODEL_FOR_INSIGHT_REGEN` or the MINI fallback chain. Pure helpers (`summarizeFilteredData`, `inferConfidenceTier`, `extractInsightCitations`, `buildInsightRegenPrompt`) live in [`server/lib/insightRegen.ts`](../../server/lib/insightRegen.ts) — see [`charting.md`](./charting.md) for the dashboard-side detail. Both [`tests/llmRoutingRegression.test.ts`](../../server/tests/llmRoutingRegression.test.ts) (`EXPECTED_CATEGORY`) and [`tests/helpers/llmStub.ts`](../../server/tests/helpers/llmStub.ts) (`DEFAULT_STUB_HANDLERS`) were updated for the new purpose so the W3.11 / W18 coverage gates stay green. 36 tests. See `docs/WAVES.md` for full entry.
- **Wave W60 · register `execute_metric_query` tool dispatcher (2026-05-18)** — closes the W56 + W57 + W58 + W59a + W59b semantic-layer chain end-to-end. W59b was read-only grounding (catalog visible in user prompt, no way to dispatch into it); W60 promotes the catalog to a first-class dispatch path. Single-bullet system-prompt edit added under the existing TOOL_ROUTER_HINT line in [`planner.ts`](../../server/lib/agents/runtime/planner.ts): names `SEMANTIC_CATALOG`, points the planner at the W59b manifest block, says PREFER `execute_metric_query` over `execute_query_plan` when a catalog metric covers the measure, and lists the three shape-only exceptions (`windowAggregations`, `percent_change`, `perDimension`) that legitimately fall through to raw `execute_query_plan`. **System-prompt edit bundled with tool registration on purpose** — the WW1+WW2+WQ2 prompt-cache breakpoints sit on the system-prompt boundary, so any byte drift there invalidates the prompt cache for the rest of the session. Doing the tool reg + the directive line in one wave means cache invalidates ONCE, not twice; W59b deliberately deferred the system-prompt edit to W60. The tool itself lives in [`executeMetricQueryTool.ts`](../../server/lib/agents/runtime/tools/executeMetricQueryTool.ts) and dispatches through the existing `execute_query_plan` executor via a closure-captured registry handle — see [`tool-registry.md`](./tool-registry.md) for the tool-side detail. Cube's predicted ~70 % share of the eventual ~300 % accuracy lift is now reachable. 28 tests. See `docs/WAVES.md` for full entry.
- **Wave W59b · wire `formatMetricCatalog` into the planner user prompt (2026-05-18)** — activates the dormant W56+W57+W58 semantic-layer chain. New thin wrapper [`buildSemanticCatalogPromptBlock(model, opts?)`](../../server/lib/semantic/prompt.ts) in `semantic/prompt.ts`: returns `""` when the model is `null` / `undefined`, else `formatMetricCatalog(model, opts) + "\n\n"`. [`planner.ts`](../../server/lib/agents/runtime/planner.ts) gets a 3-line edit — new import, new `semanticBlock = buildSemanticCatalogPromptBlock(ctx.chatDocument?.semanticModel)` constant, and an inline insertion into the user-prompt template literal so the catalog sits between `${hintsResult.block}` (WW1) and `${ragBlock}` (P-A1). System prompt is intentionally untouched: the WW1+WW2+WQ2 prompt-cache breakpoints sit on the system-prompt boundary, so any byte drift there invalidates the cache for the rest of the session. W60 (`execute_metric_query` tool registration) is the natural place to add a SEMANTIC_CATALOG directive line to the system prompt — bundling with tool reg means cache invalidates once, not twice. The catalog is read-only grounding for W59b — Cube's benchmark suggests ~30 % of the eventual ~300 % accuracy lift comes from catalog visibility alone, before tool wiring. 9 tests across `buildSemanticCatalogPromptBlock` unit cases (null/undefined → empty; populated → manifest + `\n\n`; empty-marker manifest still ends with `\n\n`; `includeHidden` pass-through; byte stability) and `planner.ts` source-inspection wiring (import path, call signature, inline template position). See `docs/WAVES.md` for full entry.
- **Wave W59a · `formatMetricCatalog` pure helper (byte-stable manifest) (2026-05-18)** — atomic setup wave that splits semantic-layer manifest rendering away from the planner prompt wiring (W59b). New module [`server/lib/semantic/prompt.ts`](../../server/lib/semantic/prompt.ts) exports `formatMetricCatalog(model, opts?)` which renders a `SemanticModel` as a byte-stable markdown block: `## Semantic catalog (vN — name)` header + instruction line + three sections (`### Metrics (count)` / `### Dimensions (count)` / `### Hierarchies (count)`) with canonical bullet blocks per entry. Collections sorted by `name.localeCompare` ascending so input ordering cannot perturb manifest bytes (prompt-cache contract). Hidden entries (`exposed: false`) filtered by default; admin UI flips `includeHidden: true`. Empty-everything models collapse to a single parenthetical marker (`_(empty — no metrics, dimensions, or hierarchies declared; fall back to raw execute_query_plan against the dataset schema)_`) rather than three empty section headings. Per-entry helpers `formatMetricLines` / `formatDimensionLines` / `formatHierarchyLines` exported so callers that need to embed a single entry in a different context (narrator-side citation expansion, admin tooltip) reuse the same byte shape. Currency hint format `currency (INR), 2 dp` parses left-to-right as `<kind> [<currency-code>] [, <decimals>]`. Helper is pure — clones-then-sorts so callers' models stay un-mutated. No `planner.ts` import yet — W59b is the call-site wave that will inline a single `formatMetricCatalog(model)` into the planner's user-prompt assembly (the inline `const user = ...` template in `planner.ts`). 19 tests across empty model, metrics section, dimensions section, hierarchies section, byte-stability, section-heading toggle, and direct helpers. See `docs/WAVES.md` for full entry.
...
- **Wave W73 · investigation mode wired into agent loop entry** — `dataAnalyzer.answerQuestion` now dispatches to [`runDeepInvestigation`](../../server/lib/agents/runtime/investigationOrchestrator.ts) before the standard `runAgentTurn` when (a) `DEEP_INVESTIGATION_ENABLED=true` AND (b) [`detectMultiPartQuestion`](../../server/lib/agents/runtime/detectMultiPartQuestion.ts) returns a multi-part intent. The dispatch decision was once owned by a dedicated pure helper (removed along with the dead `runDeepInvestigation` path — single-flow policy, invariant #6). Three fall-back paths to single-flow on any failure (gate off, throw, empty result). Emits `flow_decision` SSE row (`layer: "investigation-dispatch"`) on every fire. Default behaviour unchanged when the flag is off. See `docs/WAVES.md` for the full entry.
...
- **Wave W11 · workbench rendering + post-pivot interpretation panel** —
  the live thinking-panel row component (in
  `client/src/pages/Home/Components/ThinkingPanel.tsx`) gained a
  per-step insight annotation — an italic line on a left accent border
  directly beneath the title, so each step carries a "what this means"
  note (later refactored; the row component is now the `StepRow` label
  renderer and the per-step insight surface moved to the panel below). New
