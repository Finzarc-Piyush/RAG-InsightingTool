...
- 2026-06-30 · Wave W-WEB — **web search default-ON + broader external-claim triggers (E1).** `WEB_SEARCH_ENABLED` flipped `false→true` (free Wikipedia+GDELT providers, no key, graceful fallback; revert with `=false`). [`externalClaimDetector`](../../server/lib/agents/runtime/utils/externalClaimDetector.ts) now also fires on compare-to-market/industry and channel industry-dynamics ("quick-commerce growth") — but NOT a plain internal "GT vs Q-com volume". Narrator already weaves `source:"web"` hits via [`buildSynthesisContext`](../../server/lib/agents/runtime/buildSynthesisContext.ts) (E2, no code). See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-30 · Wave W-INSIGHT — **smarter conclusions: why + what-to-do (B1–B4).** B1: pure [`computePerformanceStanding`](../../server/lib/agents/runtime/performanceStanding.ts) (ranked complement of `computeAttentionAreas`) → `PERFORMANCE_STANDING` narrator block ([`buildPerformanceStandingBlock`](../../server/lib/agents/runtime/narratorHintsBlock.ts)) so the leader/laggard order is the stated FLOOR, not a discovery; null on ambiguous/trend shapes (L-043). B2: `ANSWER_ENVELOPE_CONTRACT` ([`sharedPrompts.ts`](../../server/lib/agents/runtime/sharedPrompts.ts)) "observation vs insight" clause — a notable movement needs a `likelyDriver` (why) or an explicit `unexplained`; recommendations act on the driver. B3: pure [`recommendationQualityGuard`](../../server/lib/recommendationQualityGuard.ts) drops common-sense filler (no lever + no number), routed through narrator `stripFindingRefs` + synthesis fallback; pure subtraction (invariant #12 safe). See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-30 · Wave W-MEM — **durable analytical memory recall (A1–A3).** Cross-turn "build-up" was leaking on RECALL, not storage. A1: the `priorInvestigations` digest now carries the turn's top-3 envelope magnitudes (`keyNumbers`) threaded via `persistMergeAssistantSessionContext({keyMagnitudes})`, `MAX_PRIOR` 5→8. A2: new `formatPriorTurnsForPrompt` ([`priorTurnState.ts`](../../server/lib/agents/runtime/priorTurnState.ts)) renders the last 3 turns' detailed findings as a `PRIOR_TURN_STATE` block, wired into [`context.ts`](../../server/lib/agents/runtime/context.ts) `formatUserAndSessionJsonBlocks` (the single-turn handle it complements was dead code since Wave B9). A3: `businessActionsAgent` gains a `PRIOR BUSINESS ACTIONS` block (`formatPriorBusinessActions`) + an advance-don't-repeat rule. See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-23 · Wave PQT1 — **pointed trend questions stop fanning out into cross-dimension breakdowns.** The depthBudget contract is now enforced at the feature sweep via pure [`shouldRunFeatureSweep`](../../server/lib/agents/runtime/dashboardFeatureSweep.ts) (was gated on `!minimalDepth`, so `standard` trend asks swept every column — L-032). Sweep fires only on explicit-dashboard / `full` depth / new [`signals.breadth`](../../server/lib/agents/runtime/queryIntentAuthority.ts) (`BREADTH_INTENT_RE`). Trend turns get deterministic same-measure coarser-grain companions instead of LLM extras ([`buildTrendTemporalCompanions`](../../server/lib/agents/runtime/visualPlanner.ts)). Day-of-week off-day fact ([`deriveWeekdayPattern`](../../server/lib/insightGenerator/weekdayPattern.ts)) feeds the chart-insight prompt + the narrator ([`buildNarratorCalendarBlock`](../../server/lib/agents/runtime/narratorHintsBlock.ts)); narrator `epistemicNotes` hardened against a bare string. See [`docs/WAVES.md`](../WAVES.md).
