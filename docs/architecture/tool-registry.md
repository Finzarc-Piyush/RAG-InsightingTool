...
- **Wave WV6-bucket (2026-05-21)** — `run_price_elasticity` now emits the categorical `effectMagnitude` bucket alongside its existing n + R² carriage on the WV2 canonical suffix. New pure helper [`bucketPriceElasticity(elasticity, significant)`](../../server/lib/agents/runtime/tools/priceElasticityTool.ts) maps fitted |β| onto the four-bucket FindingEvidence taxonomy: not significant → `"negligible"` (no measurable response); positive β anomalous → `null` (Giffen-good shape; magnitude meaningless without sign resolution; caller drops the field); non-finite β → `null`; `|β| < 0.5` → `"small"` (matches the five-step `interpretElasticity` "highly inelastic" boundary); `0.5 ≤ |β| < 1.5` → `"medium"` (collapses "inelastic" + "unit elastic" + most of "elastic"); `|β| ≥ 1.5` → `"large"` (matches "highly elastic"). Wired into the existing `headlineEvidence: FindingEvidence` builder in `runPriceElasticity` right after the n + R² fields — sets `effectMagnitude` only when `bucketPriceElasticity(...) !== null`, so positive-β fits silently omit the field rather than ship a misleading magnitude. Flows automatically through `composeFindingDetail` → WW2 extractor → WQ1 confidence classifier so "highly inelastic with tight n" (β = -0.05, p < 0.001 on N = 10,000) gets graded LOW vs "highly elastic with tight n" (β = -2.0, p < 0.001 on N = 200) gets graded HIGH — same wedge WV4-bucket closed for correlation. The bucket function lives next to `interpretElasticity` (same file, sibling concept); third sibling instance of the bucket-pattern after `bucketCorrelationR` and the WQ8 `bucketCohensD` / `bucketCramersV` family. 16 new tests in [`runPriceElasticityEffectMagnitudeWV6Bucket.test.ts`](../../server/tests/runPriceElasticityEffectMagnitudeWV6Bucket.test.ts). Existing [`runPriceElasticityFindingDetailWV6.test.ts`](../../server/tests/runPriceElasticityFindingDetailWV6.test.ts) updated in-place with `[^)]*` trailing tolerance on its two regex assertions so the new `; effect = X` token (and any future canonical evidence field) doesn't re-anchor the pin. No new convention codified — the "magnitude bucket lives next to the fit function that produces the coefficient" pattern has three sibling instances but the placement is obvious from existing structure; would codify on a fourth instance that drifts. See `docs/WAVES.md` for full entry.
- **Wave WV4-bucket (2026-05-20)** — `run_correlation` now emits the categorical effect-size bucket alongside its existing n + R² carriage. WQ8 wired `effectMagnitude` on the significance-test path; this wave wires the analogous bucket on the correlation path via Cohen's standard |r| conventions (`< 0.1` → negligible, `< 0.3` → small, `< 0.5` → medium, `≥ 0.5` → large). New pure helper [`bucketCorrelationR(r)`](../../server/lib/correlationMath.ts) lives in `correlationMath.ts` (mirroring the `bucketCohensD` / `bucketCramersV` location in `significanceTests.ts`); the [`registerTools.ts`](../../server/lib/agents/runtime/tools/registerTools.ts) WV4 block calls it on `strongest.correlation` and assigns to `evidence.effectMagnitude` under a truthy guard. The WV2 suffix shape grows from `(n = N; R² = X)` to `(n = N; R² = X; effect = <bucket>)` when the bucket is non-null; downstream WW2 extraction + WQ1 classification is the WQ8-shipped infrastructure with zero further changes. The wedge: a correlation finding like `r = 0.05 on n = 10,000` (statistically real, practically trivial) previously landed at MEDIUM because n + R² alone don't trigger LOW signals; now the `negligible` bucket forces LOW with the canonical hedge phrase. 16 new tests in [`runCorrelationEffectMagnitudeWV4Bucket.test.ts`](../../server/tests/runCorrelationEffectMagnitudeWV4Bucket.test.ts). See `docs/WAVES.md` for full entry.
- **Wave WQ8 (2026-05-20)** — `run_significance_test` now carries the categorical effect-size bucket through the WV2 evidence suffix. [`significanceTestTool.ts`](../../server/lib/agents/runtime/tools/significanceTestTool.ts) `buildEvidenceSuffix` widens its signature to `(pValue, effectiveN, effectMagnitude?)`; all three success-path call sites (welch_t, paired_t, chi_square) pass `result.effectSize.magnitude` (Cohen's d → bucket for the t-tests; Cramér's V → bucket for chi_square) through. The emitted suffix grows from `(n = N; p = X)` to `(n = N; p = X; effect = <bucket>)` when present. Downstream WW2 [`extractFindingEvidence`](../../server/lib/agents/runtime/narratorHintsBlock.ts) parses the bucket back out and WQ1's `assessConfidence` grades `"negligible"` as a hard-fail LOW signal regardless of n / p / CI — closes the "statistically significant but practically negligible" overclaim path. No new math (reuses the existing `bucketCohensD` / `bucketCramersV` in [significanceTests.ts](../../server/lib/significanceTests.ts) since F3). WV5 tests relaxed from `$`-anchored `(n = N; p = X)$` to allow the optional trailing `; effect = <bucket>` so future bucket-threshold tuning doesn't break the contract. 22 new tests in [`effectMagnitudeFindingEvidenceWQ8.test.ts`](../../server/tests/effectMagnitudeFindingEvidenceWQ8.test.ts) cover the classifier and roundtrip; see [`agent-runtime.md`](./agent-runtime.md) for the classifier-side detail. See `docs/WAVES.md` for full entry.
- **Wave W60 (2026-05-18)** — `execute_metric_query` registered. Closes the W56 → W57 → W58 → W59a → W59b semantic-layer chain end-to-end: planner emits `{ metric, breakdownBy?, filters?, sortBy?, limit? }` against catalog NAMES, the W58 compiler translates to `QueryPlanBody`, and dispatch goes through the existing `execute_query_plan` executor (DuckDB-first, in-memory fallback) — same observability, same workbench lane, same QL6 hard-fail + aggregation-floor verifiers. New file [`executeMetricQueryTool.ts`](../../server/lib/agents/runtime/tools/executeMetricQueryTool.ts): strict args schema mirroring `CompileMetricQueryInput` minus `model` (read from `ctx.exec.chatDocument?.semanticModel`); `compiledPlanToQueryPlanBody` pure helper that deep-copies arrays + drops empty optionals; `registerExecuteMetricQueryTool(registry)` closure-captures the registry, fail-fasts when no catalog is on session, dispatches via `registry.execute("execute_query_plan", { plan }, ctx)`, and re-titles the returned `workbenchArtifact` to `execute_metric_query(<metric>)`. Bundled single-bullet edit in [`planner.ts`](../../server/lib/agents/runtime/planner.ts) tells the planner to PREFER `execute_metric_query` over `execute_query_plan` when a catalog metric covers the measure (single cache invalidation rather than two — W59b deliberately deferred this). 28 tests across 7 suites: args schema validity (valid shapes + every filter op + strict-rejection cases), `compiledPlanToQueryPlanBody` roundtrip + mutation safety, `registerTools.ts` wiring source-inspection, tool registration shape source-inspection, planner.ts SEMANTIC_CATALOG rule source-inspection. See `docs/WAVES.md` for full entry.
...
- **Wave WQ7 (2026-05-18)** — `run_two_segment_compare` and `run_breakdown_ranking` now emit a canonical WV2 `FindingEvidence` suffix (`(n = N; p = X)`) from Welch's t-test on row-level metric values. [`twoSegmentCompareTool.ts`](../../server/lib/agents/runtime/tools/twoSegmentCompareTool.ts): `aggregateSegment` now also returns `values: number[]` for the segment's row-level metric values (previously discarded after the sum/mean reduction); new `buildSegmentCompareEvidence(aggregation, valuesA, valuesB)` runs Welch's t-test (A vs. B) when both segments have ≥3 finite obs and the aggregation is `mean` or `sum`; suffix concatenated onto the success-path summary. [`breakdownRankingTool.ts`](../../server/lib/agents/runtime/tools/breakdownRankingTool.ts): new `buildBreakdownRankingEvidence(frame, breakdownColumn, metricColumn, topGroupLabel, aggregation)` does a second pass over the frame splitting row-level values into the headline (top-ranked) group vs. all other groups combined, runs Welch's t-test, returns canonical suffix; only the simple-path return concatenates — composite (rankBy) path is scope-bounded out (multi-metric expression scores have ambiguous t-test shape; asserted via source-inspection test). Both tools skip count aggregation (different test shape; proportion Z-test needs a global baseline neither tool carries). Both reuse the existing `runSignificanceTest` from [significanceTests.ts](../../server/lib/significanceTests.ts) — no new math dependency. Closes the Workstream 9 deterministic-floor coverage gap for the two composite-style segment-comparison tools; 6 of 7 Workstream 9 items now shipped (WQ1–WQ7, WQ3 deferred). 15 tests across two files. See `docs/WAVES.md` for full entry.
