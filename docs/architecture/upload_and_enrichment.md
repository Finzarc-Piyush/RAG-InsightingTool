...
- **Wave W-SUGG (2026-06-30)** — sharper, domain-aware starter/follow-up questions. [`datasetProfile.ts`](../../server/lib/datasetProfile.ts) `SYSTEM_PROMPT` and the [`sessionAnalysisContext.ts`](../../server/lib/sessionAnalysisContext.ts) `SEED_SYSTEM` now bias `suggestedQuestions`/`suggestedFollowUps` toward comparisons / movements / outliers in real business nouns; the seed gains a `domainContext` param fed from `uploadQueue`'s `datasetProfileDomainContext`. Still routed through `stripOrQuestions`. See `docs/WAVES.md`.

- **dataOps god-file decomposition · Batch 17 (ARCH-2 / CQ-2)** — Seven more HEAVY inline branches of [`executeDataOperation`](../../server/lib/dataOps/dataOpsOrchestrator.ts) moved VERBATIM into typed handlers under [`server/lib/dataOps/handlers/`](../../server/lib/dataOps/handlers/): `preview`, `treat_outliers`, `create_column`, `create_derived_column`, `rename_column`, `filter`, `revert`. Each orchestrator `case` is now a thin `return await handle<Op>({…})`; the branch-private AI helpers (`extractColumnDetails`, `extractDerivedColumnDetails`) moved alongside their owning handler. The column-creating handlers preserve the exact persist→context-update interleaving (`dataOpsContext.lastCreatedColumn` written via a second save). The orchestrator dropped from ~4141 → ~3179 LOC; the largest remaining inline branch is `remove_rows` (~77 lines). The hermetic `__setFetchFnForTesting` seam now also covers the `/preview`, `/create-derived-column`, and `/treat-outliers` Python calls (routed through `callFetchFn`, matching that seam's documented contract). New integration coverage in [`dataOpsRestBranchesIntegration.test.ts`](../../server/tests/dataOpsRestBranchesIntegration.test.ts) (happy-path + short-circuit per branch, through the Cosmos double). See `docs/WAVES.md` for the full entry.
