...
- 2026-06-30 · **Waves W1–W12 — finance-aware insighting: never SUM a ratio; never report an accounting identity as causal.** New single authority [`financeMetricAuthority.ts`](../../server/lib/financeMetricAuthority.ts) (invariant **I13**) owns metric semantics — a `FINANCE_TERMS` registry (kind, additivity, each ratio's numerator/denominator) plus the identity graph. The three drifting rate-regexes (`chartSpecCompiler.NON_ADDITIVE_METRIC_RX`, `dashboardFeatureSweep.RATE_METRIC_RX`, the pivot's `isRateMetric`) now delegate to `isNonAdditiveMetric` (which, via `normalizeMetricTokens`, finally matches the literal `%` they all missed). [`chartGenerator.processChartData`](../../server/lib/chartGenerator.ts) replaces its `none→sum` coercion with a metric-aware `resolveAggregation`: a non-additive metric is **weighted-averaged by its denominator** (Σ(GC%·NR)/Σ NR — scale-preserving) or meaned, NEVER summed, in both `aggregateData` and `pivotLongToWideBar`; a non-additive series is forced `grouped`, never `stacked`. Additivity is stamped at enrichment onto `DataSummary.columns[].additivity` ([`uploadQueue`](../../server/utils/uploadQueue.ts)) and `inferModel` stops emitting `SUM(GC%)`. [`finishChartSpec`](../../server/lib/chartSpecFinish.ts) adds a "weighted average, not a sum" caption. Causation side: a per-turn identity graph on `ctx.identityGraph` filters definitional pairs out of [`correlationAnalyzer`](../../server/lib/correlationAnalyzer.ts) (new `structural_identity_filtered` diagnostic), a `NO_STRUCTURAL_IDENTITY` predicate in [`verifierCausalCheck`](../../server/lib/agents/runtime/verifierCausalCheck.ts) blocks "GC% is impacted by NR" tautologies (decomposition-exempt), and an `ACCOUNTING IDENTITIES` prompt block ships the gate with the permission. ADR [`centralized-metric-semantics`](../decisions/centralized-metric-semantics.md), convention [`metric-additivity`](../conventions/metric-additivity.md), lessons L-039/L-040/L-041. See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-29 · **Waves W-EXP-DECK1…6 — dashboard PowerPoint export reliably produces a full deck (verifier becomes advisory).** The export deck verifier ([deckPlanVerifier.ts](../../server/lib/agents/runtime/deckPlanVerifier.ts)) was an all-or-nothing gate: `buildAndVerifyDeckPlan` ([buildDashboardDeck.ts](../../server/lib/exports/buildDashboardDeck.ts)) discarded a full multi-slide deck and shipped the 3-slide stub if any one slide tripped any one of 6 rules even after one repair — so a 32-chart dashboard collapsed to 3 slides every time. Now: deterministic `autoRepairDeckPlan` fixes the mechanical rules (slide ordering + short speaker notes) with no LLM; the verifier DRIVES at most one LLM repair but the best renderable plan is SHIPPED regardless (`buildDeck.shippedWithResidualIssues`; the old verifier-failed-after-repair null-path retired). `buildFallbackDeckPlan` now emits a real deck — one `ChartWithInsight` per chart (resolvable `s{i}c{j}` ids via the exported `chartIdFor`) + envelope chrome, capped at 30 with overflow → Appendix. Planner prompt curates / Appendix-overflows for many-chart dashboards; `maxTokens` 12K→16K. No `slideDeckPlanSchema` change. Lesson L-037. See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-26 · **Wave W-XAX2 — recharts x-axis labels now distribute evenly (floored-stride fix).** [`evenlySpacedDataKeys`](../../client/src/lib/chartRechartsShared.ts) used a floored integer stride that collapses to 1 when the label budget lands in `(n/2, n)` (e.g. 25 of 48), cramming the first N category labels onto the left with a blank gap + lone final label (dashboard inline bar tiles; the wider fullscreen modal landed on a safe budget). Now uses the rounded float stride of its visx sibling `pickEvenlySpacedTicks` (full-range spread, first+last pinned, dedup by index) — fixing the inline tile, dashboard modal, and chat modal at once. Test [`chartRechartsShared.vitest.test.ts`](../../client/src/lib/chartRechartsShared.vitest.test.ts); lesson L-035. See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-24 · **Off-day handling (weekday column + non-blocking exclude).** New 7th MATERIALIZED temporal grain `day_of_week` → a `Day of week · <Date>` column storing **pure-text** weekday names; Mon→Sun ordering via the single [`weekday.ts`](../../server/shared/weekday.ts) (`weekdayRank`) taught to BOTH chart-sort authorities ([`chartSort.ts`](../../server/shared/chartSort.ts) + the pivot's `compareTemporalOrLocale`). Cyclical, so it is kept OUT of trend-grain selection (`GRAINS_FINE_TO_COARSE`/`DEFAULT_FACET_PREFERENCE`). The chart-preview + key-insight endpoints surface a transient `offDayHint` (reusing [`deriveWeekdayPattern`](../../server/lib/insightGenerator/weekdayPattern.ts), now also exporting `computeOffDayHint`/`filterRowsByExcludedWeekdays`); the [`OffDayAffordance`](../../client/src/components/charts/OffDayAffordance.tsx) pill in [`ChartBuilderDialog`](../../client/src/components/ChartBuilderDialog.tsx) offers **per-chart** exclusion (`ChartSpec.excludedWeekdays`, filtered BEFORE aggregation → working-day averages, no math change) + an **"Apply to all charts"** escalation via a new `notIn` active-filter condition. Convention [`off-day-handling.md`](../conventions/off-day-handling.md), lesson L-034. See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-24 · **Build-Chart preview auto-grain now obeys the temporal grain authority (invariant #11).** The manual chart builder ([`ChartBuilderDialog`](../../client/src/components/ChartBuilderDialog.tsx)) posts `{x, y, aggregate}` with no `grain` to `POST /api/sessions/:id/chart-preview` → `processChartData`'s aggregate branch → [`inferOptimalPeriodForChartColumn`](../../server/lib/chartDownsampling.ts) → `determineOptimalPeriod`. That helper rolled its OWN span ladder (`days > 14 → week`), so picking the raw `Date` column on a single month of daily data bucketed it to WEEK (`2026-W14`…) while every agent-built trend chart kept the same data at DAY grain. Fixed by deleting the local ladder and delegating to the authority's exported span primitive [`pickTrendGrainForSpan`](../../server/lib/temporalGrainAuthority.ts) (≤90d → day, ≤1y → week, ≤5y → month, else quarter); `determineOptimalPeriod` now derives `spanDays` + `distinctDayCount` from the parsed dates and returns its token (a subset of `DatePeriod`). The invariant spec forbids `pickTrendGrainForSpan` only in the four higher-level builders (visualPlanner / dashboardFeatureSweep / chartFromTable / periodColumnResolver) that must use the richer `resolveTrendGrain`; the preview path has no NL intent and buckets a raw date column by span, so the span primitive is the correct level. Test [`chartDownsamplingOptimalPeriod.test.ts`](../../server/tests/chartDownsamplingOptimalPeriod.test.ts). See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-24 · **Waves B2–B5 — continuous TIME columns are binned before they become a chart axis.** A "`<metric> (avg) by <dim>`" chart keyed each bar by the raw dim value (`aggregateData`: `key = String(row[groupBy])`), so a **time-of-day** column (Clock-In Time, `08:50:49`…) or a **duration** column (Working Hrs, `03:16:55`…) rendered one bar per distinct per-second value. Calendar dates were already bucketed by the temporal grain authority; time-of-day/duration had no equivalent. New pure authority [`continuousDimensionBucket.ts`](../../server/lib/continuousDimensionBucket.ts) (`planContinuousDimensionBucket` → time-of-day = hour-of-day bands auto-refining to 30/15-min when clustered; duration = round whole-hour ranges; `applyContinuousDimensionBucket` + the `bucketContinuousXForSpec` one-liner). It is applied as a **row rewrite** (raw value → lower-bound-leading bucket label `08:00–09:00` / `3h–4h`) BEFORE compile/aggregate — the same shape as the sweep's `bucketRowsTopN`/`deriveDimensionBucket`. Wired into every "by X" builder: [`chartFromTable`](../../server/lib/agents/runtime/chartFromTable.ts) (BEFORE `X_LABEL_CARDINALITY_CAP`, so a per-second column is binned not suppressed), [`dashboardFeatureSweep`](../../server/lib/agents/runtime/dashboardFeatureSweep.ts) (before the cardinality regime), [`visualPlanner`](../../server/lib/agents/runtime/visualPlanner.ts), [`agentLoopDeferredCharts`](../../server/lib/agents/runtime/agentLoopDeferredCharts.ts), and the `build_chart` tool ([`registerTools`](../../server/lib/agents/runtime/tools/registerTools.ts)). Sort/labels are free: labels lead with their lower bound so `shared/chartSort`'s `detectAxisOrdered`/`bucketLeadingNumber` orders the axis ascending. Decision is annotation-driven (`timeOfDay`/`duration`) with sample-shape fallback; the annotation-blind `aggregateData` is untouched (lessons L-019/L-024/L-032). Scope = time-of-day + duration only (generic numeric binning intentionally excluded so 1–12 month / 1–5 rating axes are never re-bucketed). Convention [`continuous-dimension-bucketing`](../conventions/continuous-dimension-bucketing.md). See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-23 · **Wave PQT1 — a pointed trend question gets the same measure at coarser grains, not cross-dimension breakdowns; plus day-of-week grounding.** The cross-dimension feature sweep now runs only via the pure [`shouldRunFeatureSweep`](../../server/lib/agents/runtime/dashboardFeatureSweep.ts) gate (explicit-dashboard / `full` depth / `signals.breadth`), so a `standard` trend ask no longer fans out one chart per column. [`buildTrendTemporalCompanions`](../../server/lib/agents/runtime/visualPlanner.ts) replaces the LLM proposer for trend turns with span-gated Day→Week→Month companions of the SAME measure (re-bucketed from the primary chart's own points). [`deriveWeekdayPattern`](../../server/lib/insightGenerator/weekdayPattern.ts) feeds a data-derived off-day fact into the Key-Insight prompt so the WHY names the weekly rhythm. L-032. See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-22 · **Wave W-XAX1 — x-axis label density is fully dynamic (rotate-to-fit), no hardcoded cap, uniform on every surface.** One shared authority [`xAxisTickBudget`](../../client/src/lib/charts/xAxisLabelCap.ts) (`{max, rotateDeg}`) drives count + tilt from area + data-point count (`min(width-fit, dataCount)`, ABS guard 60→200 as a DOM backstop only); all 7 visx renderers + recharts surfaces delegate to it, long/many labels tilt −45°, [`useContainerWidth`](../../client/src/hooks/useContainerWidth.ts) uses `useLayoutEffect`, SSR export drops the `>18` threshold for ECharts `interval:"auto"`. Convention [`dynamic-x-axis-label-density`](../conventions/dynamic-x-axis-label-density.md). See [`docs/WAVES.md`](../WAVES.md).
...
- 2026-06-23 · **Waves NCF1 + TBN1.** The shared [`ChartInsightBody`](../../client/src/components/charts/ChartInsightBody.tsx) dropped the `businessCommentary` "Business context:" block (the field is retired everywhere — see [`no-domain-context-framing.md`](../conventions/no-domain-context-framing.md)) and now renders **Why / Do at normal size** (no `text-[12px]`; Why no longer italic). The shared sort authority [`chartSort.ts`](../../server/shared/chartSort.ts) gained an additive `limit: {mode:'top'|'bottom', n}` option + `selectBottomNByValue` (selection by value, decoupled from display sort); fullscreen bar charts with >10 categories expose a "Show" Top/Bottom-N control ([`ChartLimitControl`](../../client/src/components/charts/ChartLimitControl.tsx)) in `ChartModal` / `ChartOnlyModal` (ephemeral; no schema change). See [`docs/WAVES.md`](../WAVES.md).

- 2026-06-26 · **Waves DA1–DA4.** Dashboards anchor on the metric the user NAMED (`findMetricMentionedInQuestion` in [`columnMatcher.ts`](../../server/lib/agents/utils/columnMatcher.ts) feeds `ensureDashboardOutcomeMetric`/`resolveBreadthOutcomeMetric`), and a pointed ask no longer auto-fans every boolean indicator (`ensureDashboardOutlineMetrics`) — so a "PJP dashboard" stops surfacing Compliance. The anchor's trend now renders a **multi-grain ladder** (`resolveTrendGrainLadder` in [`temporalGrainAuthority.ts`](../../server/lib/temporalGrainAuthority.ts) — month → week+day, year → quarter+month) applied post-merge by [`applyTrendGrainLadder`](../../server/lib/agents/runtime/trendGrainLadder.ts), replacing the hardcoded `[week,month]` companions. Multi-series bars default **grouped** for rates / **stacked** for additive measures ([`chartSpecCompiler`](../../server/lib/chartSpecCompiler.ts) + proposer `barLayout`), and [`buildAnchorComparisonCharts`](../../server/lib/agents/runtime/metricComparisonCharts.ts) emits "anchor vs named-secondary by dim" grouped tiles. Convention [`trend-grain-ladder.md`](../conventions/trend-grain-ladder.md). See [`docs/WAVES.md`](../WAVES.md).

- 2026-06-26 · **Wave W-INS1.** The dashboard insight-regen ([`useInsightRegen.ts`](../../client/src/pages/Dashboard/hooks/useInsightRegen.ts)) and the on-open insight self-heal ([`useEnsureDashboardInsights.ts`](../../client/src/pages/Dashboard/hooks/useEnsureDashboardInsights.ts)) use raw `fetch`, which bypasses the axios `apiClient` auth interceptor — both now spread the MSAL Bearer token, so "Generate insight" and the self-heal stop 401ing with "Missing Authorization" and bare "No insight yet" tiles actually heal. Part of a seven-site raw-fetch sweep (new convention [`authed-raw-fetch.md`](../conventions/authed-raw-fetch.md), lesson L-036). See [`docs/WAVES.md`](../WAVES.md).

- 2026-06-29 · **Wave W-1PT1.** A trendline needs >1 point. New shared authority [`server/shared/chartValidity.ts`](../../server/shared/chartValidity.ts) (`isDegenerateTrendChart`; client shim [`chartValidity.ts`](../../client/src/shared/chartValidity.ts)) drops a `line`/`area`/`scatter` chart whose materialized x-axis has <2 distinct points — server-side in [`finalizeMergedCharts`](../../server/lib/agents/runtime/agentLoop/finalizeCharts.ts) before persist, client-side in the [`DashboardView`](../../client/src/pages/Dashboard/Components/DashboardView.tsx)/[`MessageBubble`](../../client/src/pages/Home/Components/MessageBubble.tsx) tile lists so already-saved single-dot tiles vanish on view. Convention [`trend-needs-two-points.md`](../conventions/trend-needs-two-points.md), lesson L-038. See [`docs/WAVES.md`](../WAVES.md).

- 2026-06-30 · **Waves W-BLIM1–4 · honest bar charts (no more silent "top few + bottom few").** The dashboard feature sweep ([`dashboardFeatureSweep.ts`](../../server/lib/agents/runtime/dashboardFeatureSweep.ts)) **deleted `bucketTopAndBottom`** — the best-K+worst-K merge that kept top-8+bottom-8=16, **dropped the middle**, and baked a 16-row array into the persisted `data` (so the chart read as a continuous ranking AND the "View all 16 as a sortable table" link couldn't reach the rest). Now a high-cardinality categorical bar dim ≤ `EMBED_CAP` (300) embeds the **FULL** category set and bakes a durable, honest **Top-15** display default (new `limit:{mode,n}` field on `chartSpecSchema`, decoupled from `sort`/`maxRows`); only past EMBED_CAP does it fall back to a **visible** top-15 + "Other" rollup. [`ChartRenderer`](../../client/src/pages/Home/Components/ChartRenderer.tsx) applies `chart.limit` (via the shared `applyChartSort` `limit`) **before** the width cap, flooring `compactBarLimit` at `limit.n` so the width cap thins only LABELS — the pivot/"View all" path keeps the limit-free `sortedSpec` so every record stays reachable. The ephemeral fullscreen [`ChartLimitControl`](../../client/src/components/charts/ChartLimitControl.tsx) (TBN1) is now also an **inline, durable** tile control: rendered in [`ChartTileBody`](../../client/src/pages/Dashboard/Components/ChartTileBody.tsx) (bar + >10 categories) with a live "Top N of M" caption, persisted via the extended `PATCH /api/dashboards/:id/charts/:idx` dual-write (mirrors `sort`); the modals seed from `chart.limit`. Convention [`chart-limit-durable.md`](../conventions/chart-limit-durable.md). See [`docs/WAVES.md`](../WAVES.md).
- 2026-06-30 · **Wave W-CHARTDIVE · per-chart "Investigate further" → full deep-dive.** Each chat chart tile ([`MessageBubble.tsx`](../../client/src/pages/Home/Components/MessageBubble.tsx)) gains a Telescope button that SUBMITS a deep-dive question (new pure [`buildChartInvestigationPrompt`](../../client/src/lib/charts/investigateQuestion.ts)) via a new `onInvestigateChart` prop wired to [`ChatInterface`](../../client/src/pages/Home/Components/ChatInterface.tsx)'s `handleInvestigateChart` (`onSendMessage`, isLoading-gated) — distinct from the pre-fill-only `onSuggestedQuestionClick`. The prompt is diagnostic+strategic so the server's `queryIntentAuthority` runs it at `depthBudget=full` (causal tools + business actions), no new flag. See [`docs/WAVES.md`](../WAVES.md).
